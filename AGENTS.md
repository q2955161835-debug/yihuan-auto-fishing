...
- 当前阶段：用户已指定当前已验证版本为 `main` 基线并完成私有仓库推送；真实高品质鱼的自适应窄绿区识别、A/D 中心控制及结算点击已分别通过。当前隔离分支保留稳定屏幕键盘布局，在顶部进度条稳定后干净消失时直接启动 2.10～2.60 秒随机计时，到时点击安全空白区域；结算页视觉输出不再参与状态迁移或点击，点击正常返回即计数。完整单轮计数未通过前不合并。
...
主数据流：绑定游戏窗口 → 启动或复用 Windows 屏幕键盘并固定到游戏显示器左下角 → 点击开始并进行 3 秒手动切回游戏倒计时 → 验证游戏前台 → 获取客户区与显示器 → 30 帧/秒截图 → 识别器返回结构化结果 → 状态机决定输入 → 鼠标点击/按住屏幕键盘 F/A/D → 顶部进度条干净消失后随机计时并点击游戏安全空白处 → 界面展示状态。应用启动后同时建立完整运行记录目录：每个已处理客户端帧写入结构化观测、前后状态和低分辨率截图；键鼠请求与系统输入返回值写入同一事件流。暂停后点击继续同样进行 3 秒手动切回倒计时并复核前台。任何异常先抬起鼠标并释放 A/D，再暂停。
...
等待结算不得依赖鱼种、品质、卡片颜色或其他结算页视觉特征。累计至少 15 个可靠控制帧且顶部进度条连续三帧干净消失后，按真实运行 `run-20260713T070804371619Z` 的时间证据，从消失确认时刻随机等待 2.10～2.60 秒，再选择未被屏幕键盘遮挡的游戏客户区安全空白点点击一次；历史样本中鱼获页约 1.86 秒稳定，已知成功点击约 2.34 秒。输入服务点击正常返回后立即记录 `signal=click_succeeded`，把当前轮计为成功并进入完成或轮间等待，不再识别结算页、卡片消失、右下初始鱼钩或其他结束画面，也不重试；点击抛错时不得计数，必须走既有 `E_INPUT` 安全暂停。所有候选点被屏幕键盘遮挡时以 `E_OSK` 暂停。随机计划、尝试坐标、计时触发和点击成功必须写入完整运行日志；等待结算期间暂停再继续时重新抽取完整随机延迟。
...
- `codex/feat-runtime-logging` 分支当前代码基线为 309 个 pytest 测试通过；前一发布物的单文件构建、管理员清单校验和启动/响应/关闭烟雾通过，定时结算点击的新发布物尚待构建。真实高品质鱼控制与一次结算点击已通过，稳定屏幕键盘尺寸、定时点击和点击即计数的组合流程仍待实机执行，这不替代真实游戏人工验收。
