...
- 当前阶段：用户已指定当前已验证版本为 `main` 基线并完成私有仓库推送；真实高品质鱼的自适应窄绿区识别、A/D 中心控制及真实鱼获卡片关闭已通过。关闭后因旧就绪图标规则未计数的问题已改为连续三帧卡片消失确认并完成离线测试；完整单轮计数未通过前不合并。
...
真实鱼获卡片必须同时满足中央卡片颜色结构与顶部奖励栏结构，紫色转场漩涡不得单独触发。确认卡片后首次点击随机等待 0.18～0.42 秒；卡片仍存在时以 0.40～0.80 秒随机间隔重试，累计最多三次。每次点击前必须重新确认卡片并选择未被屏幕键盘遮挡的游戏客户区安全点；点击后卡片连续三帧消失即确认关闭并计数，少于三帧后卡片重现必须清零重新确认，现有就绪画面识别仍可提前确认。三次后卡片仍存在以 `E_RESULT_DISMISS` 暂停；所有候选点被屏幕键盘遮挡以 `E_OSK` 暂停。计划、尝试坐标、连续消失确认、重试和最终失败必须写入完整运行日志。
...
- `codex/feat-runtime-logging` 分支当前离线基线为 312 个 pytest 测试通过；上一发布物的单文件构建、管理员清单校验和启动/响应/关闭烟雾通过。真实高品质鱼控制与鱼获卡片关闭已通过，连续三帧消失后的计数仍待新发布物执行，这不替代真实游戏人工验收。
