...
真实《异环》上钩提示位于游戏客户区右下 `BITE_ROI=(0.89,0.79)-(0.99,0.99)` 的蓝白圆环，左上文字提示不是可用的上钩信号。第一次 F 后以该区域建立基线，前 45 帧（约 1.5 秒）忽略抛竿动画；之后要求蓝色比例相对基线增加大于 `0.03`，且白色比例增加大于 `0.03` 或 Canny 边缘比例增加大于 `0.02`，连续两帧才确认。变更此条件必须以运行截图回放验证：冷却期零触发、上钩前无误触发、提示出现后连续两帧触发。
