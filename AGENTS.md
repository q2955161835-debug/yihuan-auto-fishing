...
- 当前阶段：用户已指定当前已验证版本为 `main` 基线并完成私有仓库推送；真实高品质鱼的自适应窄绿区识别、A/D 中心控制及蓝色鱼获卡片关闭已通过。青绿色鱼获卡片与真实右下初始鱼钩识别已完成真实帧回归和离线测试；完整单轮计数未通过前不合并。
...
- `codex/feat-runtime-logging` 分支当前离线基线为 315 个 pytest 测试通过；上一发布物的单文件构建、管理员清单校验和启动/响应/关闭烟雾通过。真实高品质鱼控制与蓝色鱼获卡片关闭已通过，青绿色鱼获卡片点击及右下初始鱼钩计数仍待新发布物执行，这不替代真实游戏人工验收。
