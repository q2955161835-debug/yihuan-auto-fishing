...
- 当前阶段：用户已指定当前已验证版本为 `main` 基线并完成私有仓库推送；真实高品质鱼的自适应窄绿区识别、A/D 中心控制及蓝色鱼获卡片关闭已通过。按用户要求，生产识别已回退到鱼获点击成功的 `27fa29a` 行为，保留稳定屏幕键盘布局，并把系统点击正常返回设为本轮完成边界；完整单轮计数未通过前不合并。
...
真实鱼获卡片按已成功点击的 `27fa29a` 规则，必须同时满足中央蓝色圆盘、白色环形细节、中央暗度与顶部奖励栏结构；紫色转场不得单独触发。确认卡片后首次点击随机等待 0.18～0.42 秒，每次点击前重新确认卡片并选择未被屏幕键盘遮挡的游戏客户区安全点。输入服务点击正常返回后立即记录 `signal=click_succeeded`，把当前轮计为成功并进入完成或轮间等待，不再识别卡片消失、右下初始鱼钩或其他结束画面，也不再重试；点击抛错时不得计数，必须走既有 `E_INPUT` 安全暂停。所有候选点被屏幕键盘遮挡时以 `E_OSK` 暂停。计划、尝试坐标和点击成功必须写入完整运行日志。
...
- `codex/feat-runtime-logging` 分支当前源码离线基线为 313 个 pytest 测试通过；最近发布物仍为 326 项构建门、管理员清单校验和启动/响应/关闭烟雾通过，等待历史识别回退与点击即计数版本重建。真实高品质鱼控制与蓝色鱼获卡片关闭已通过，稳定屏幕键盘尺寸和点击即计数仍待新发布物执行，这不替代真实游戏人工验收。
