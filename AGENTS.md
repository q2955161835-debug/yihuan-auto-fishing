...
- 当前阶段：用户已指定当前已验证版本为 `main` 基线并完成私有仓库推送；进度槽一维重建、稳定结束切换、自适应窄绿区与完整运行记录已在隔离分支完成 303 项离线自动测试，等待新单文件构建以及真实《异环》高品质鱼、结算和计数验收后合并。
...
- `codex/feat-runtime-logging` 分支当前离线基线为 303 个 pytest 测试通过；自适应窄绿区新发布物与真实高品质鱼、结算和计数仍待执行，这不替代真实游戏人工验收。
