...
真实《异环》上钩提示位于游戏客户区右下 `BITE_ROI=(0.89,0.79)-(0.99,0.99)` 的蓝白圆环，左上文字提示不是可用的上钩信号。第一次 F 后以该区域建立基线，前 45 帧（约 1.5 秒）忽略抛竿动画；之后要求蓝色比例相对基线增加大于 `0.03`，且白色比例增加大于 `0.03` 或 Canny 边缘比例增加大于 `0.02`，连续两帧才确认。顶部进度条必须将细黄色标记（高度至少 ROI 的 6%）配对到相邻绿色条的水平范围，不能把左侧圆形 UI 当标记；控制目标是让黄色标记回到绿色区域中心，中心偏差大于绿色宽度的 10% 时才按键：黄标偏左按 D，偏右按 A，进入中心死区立即释放。控制完成后的中央“鱼上钩了，快点击按钮上鱼！”横幅在 `REEL_PROMPT_ROI=(0.22,0.16)-(0.60,0.24)` 连续两帧出现后，先释放 A/D 再轻点 F 并进入结算等待。变更此条件必须以运行截图回放验证。
