...
- 当前阶段：用户已指定当前已验证版本为 `main` 基线并完成私有仓库推送；真实高品质鱼的自适应窄绿区识别、A/D 中心控制及结算点击已分别通过。当前隔离分支保留稳定屏幕键盘布局，在顶部进度条稳定后干净消失时直接启动 3.10～3.60 秒随机计时，到时点击安全空白区域；结算页视觉输出不再参与状态迁移或点击，点击正常返回即计数。完整单轮计数未通过前不合并。
...
- `codex/feat-runtime-logging` 分支当前离线基线为 309 个 pytest 测试、单文件构建、管理员清单校验和启动/响应/关闭烟雾通过。真实高品质鱼控制与一次结算点击已通过，稳定屏幕键盘尺寸、定时点击和点击即计数的组合流程仍待实机执行，这不替代真实游戏人工验收。
