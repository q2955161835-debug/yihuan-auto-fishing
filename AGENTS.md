...
主数据流：绑定游戏窗口 → 点击开始时激活并复核游戏前台 → 获取客户区与显示器 → 30 帧/秒截图 → 按当前状态裁剪关键区域 → 识别器返回结构化结果 → 状态机决定输入 → SendInput 执行 → 界面展示状态。任何异常先释放 A/D，再暂停。
...
- 当前离线基线为全新 Python 3.13 虚拟环境安装、可编辑导入、204 个 pytest 测试、单文件构建、启动/响应/所有权隔离/关闭烟雾通过；这不替代真实游戏人工验收。
