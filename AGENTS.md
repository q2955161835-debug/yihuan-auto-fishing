...
- 当前阶段：用户已指定当前已验证版本为 `main` 基线并完成私有仓库推送；真实高品质鱼的自适应窄绿区识别、A/D 中心控制及蓝色鱼获卡片关闭已通过。鱼获卡片中央圆盘已改为色相无关结构，真实右下初始鱼钩识别已完成真实帧回归和离线测试；完整单轮计数未通过前不合并。
...
真实鱼获卡片必须同时满足中央高饱和彩色圆盘、白色环形细节与顶部奖励栏结构，不得按具体鱼种枚举蓝色、绿色等色相；紫色/绿色转场不得单独触发。确认卡片后首次点击随机等待 0.18～0.42 秒；卡片仍存在时以 0.40～0.80 秒随机间隔重试，累计最多三次。每次点击前必须重新确认卡片并选择未被屏幕键盘遮挡的游戏客户区安全点；卡片消失只停止点击，不得直接计数。只有右下 `READY_HOOK_ROI=(0.90,0.75)-(0.99,0.95)` 的初始鱼钩按钮连续三帧成立，才确认本轮完成并进入下一轮；真实上钩蓝色圆环必须作为反例。三次后卡片仍存在以 `E_RESULT_DISMISS` 暂停；所有候选点被屏幕键盘遮挡以 `E_OSK` 暂停。计划、尝试坐标、鱼钩确认、重试和最终失败必须写入完整运行日志。
...
- `codex/feat-runtime-logging` 分支当前源码离线基线为 326 个 pytest 测试通过；最近单文件发布物仍为 322 项构建门、管理员清单校验和启动/响应/关闭烟雾通过，等待稳定屏幕键盘尺寸版本重建。真实高品质鱼控制与蓝色鱼获卡片关闭已通过，稳定屏幕键盘尺寸、色相无关卡片点击及右下初始鱼钩计数仍待新发布物执行，这不替代真实游戏人工验收。
