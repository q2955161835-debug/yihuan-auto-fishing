...
- 当前阶段：用户已指定当前已验证版本为 `main` 基线并完成私有仓库推送；进度槽一维重建与完整运行记录已在隔离分支完成 289 项离线自动测试，等待重新构建和真实《异环》单轮验收后合并。
...
真实《异环》上钩提示位于游戏客户区右下 `BITE_ROI=(0.89,0.79)-(0.99,0.99)` 的蓝白圆环，左上文字提示不是可用的上钩信号。第一次 F 后以该区域建立基线，前 45 帧（约 1.5 秒）忽略抛竿动画；之后要求蓝色比例相对基线增加大于 `0.03`，且白色比例增加大于 `0.03` 或 Canny 边缘比例增加大于 `0.02`，连续两帧才确认。顶部进度识别固定使用 `TOP_ROI` 局部纵向 40%～52% 的五条扫描线：先在槽带确认黄色标记，再将黄标两侧同一水平槽内的绿色段重建为完整逻辑绿区，至少三条扫描线边界一致才输出观测；不得恢复为单个二维绿色连通块宽度判断。相对最近可靠绿区中心跳变超过槽宽 20% 的候选等待下一帧确认，确认期间返回空观测并释放 A/D；历史观测不得直接驱动输入。控制目标是让黄色标记回到绿色区域中心，中心偏差大于绿色宽度的 10% 时才按键：黄标偏左按 D，偏右按 A，进入中心死区立即释放。中央提示横幅只作为运行诊断字段，不得在进度控制阶段触发额外 F 或提前进入结算。变更这些条件必须以真实失败夹具和合成扫动序列回放验证。
...
- `codex/feat-runtime-logging` 分支当前离线源码基线为 289 个 pytest 测试通过；新发布物构建和真实游戏单轮仍待本阶段执行，这不替代真实游戏人工验收。
