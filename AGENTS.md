...
真实《异环》上钩提示位于游戏客户区左上 `BITE_ROI=(0.23,0.02)-(0.32,0.14)`，不是右下准备区域。上钩检测必须在该区域同时满足白色比例 `0.02~0.20`、Canny 边缘比例大于 `0.02`、暗色比例 `0.05~0.80`，并连续两帧确认；不得再依赖抛竿前截图差分或“蓝色增加”。变更此条件必须以运行截图回放验证：上钩前无误触发、提示出现后连续两帧触发。
